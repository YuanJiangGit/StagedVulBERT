...
class Model(RobertaForSequenceClassification):
    ...
    def forward(self, tokens_features, labels_c=None, labels_f=None):
        ...
        x = self.SETransformer.roberta(inputs_embeds=padded_rows_rep, attention_mask=attn_mask,
                                       output_attentions=True)

        logits = self.classifier(x[0])

        if labels_c is None and labels_f is None:
            logits_c = logits[:, 0, :]
            prob = torch.softmax(logits_c, dim=-1)
            return prob[:, 1] > 0.5
        ...
        # loss_c = torch.zeros(1).to(self.args.device)
        loss_f = torch.zeros(1).to(self.args.device)
        # if labels_c is not None:
        #     loss_c = loss_fct(logits_c, labels_c)
        if labels_f is not None and labels_c.sum() > 0:
            loss_fct = nn.CrossEntropyLoss(ignore_index=-1)
            logits_f = logits_f[labels_c.bool()].reshape(-1, 2)
            labels_f = labels_f[labels_c.bool()][:, 1:max_row_num].reshape(-1)
