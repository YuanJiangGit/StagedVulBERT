...
class MSLMTask(nn.Module):
    ...
    def forward(self, se_outputs, inputs_ids, mask_row_pos, first_stoken_mask):
        ...
        # 计算masked语句的最大长度
        trg_len = max(len(x) for x in true_token_ids)
        # padding true_token_ids，并转化为 PyTorch Tensor 矩阵
        padded_true_token_ids = [[self.tokenizer.cls_token_id] + x.tolist() + [self.tokenizer.pad_token_id] * (trg_len - len(x))
            + [self.tokenizer.sep_token_id] for x in true_token_ids]
        trg = torch.tensor(padded_true_token_ids).to(device)
        # +2是（CLS和SEP）
        trg_len += 2

        # 提取了与非padding位置相对应的特征，masked_se_features [masked row length, hid dim]
        masked_se_features = se_outputs[masked_pos].to(device)
...
class Model(nn.Module):
    ...
    def forward(self, inputs_ids, first_stoken_mask, row2row_mask, attn_mask, masked_source_ids, mask_row_pos,
        ...
        rows_rep = [avg_outputs[i, first_stoken_mask[i]].clone() for i in range(batch)]
        ...
        max_row_num = padded_rows_rep.clone().shape[1]
        attn_mask = [[1] * rows.shape[0] + [0] * (max_row_num - rows.shape[0]) for rows in rows_rep]
